...
const SignUp: React.FC = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!error) {
        // Redirect after successful signup (or after email verification)
        console.log(data);
      }
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div className="px-4 py-8 md:p-10 sm:p-4 border border-black rounded-xl bg-black/50 shadow w-1/2 sm:w-full sm:max-w-sm md:max-w-md xl:max-w-2xl">
        ...
        <form className="flex flex-col gap-y-4 md:gap-y-6" onSubmit={handleSignUp}>
          <input
            type="text"
            placeholder="Name"
